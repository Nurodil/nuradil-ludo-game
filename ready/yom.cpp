...
bool isTokenAvailable(int player, char choice, LUDO& l);
		...
		do {
			...
			for (int i = 0; i < l.noOfTokens; i++)
				if (l.tokens[player][i] == choice)
					index = i;
			if (index == -1 || l.TokenOut[player][index] != choice)
				cout << "Bu piyonu secemezsiniz! Baska birini deneyin : ";
		} while (index == -1 || l.TokenOut[player][index] != choice);
...
void OpenSaveGame() {
	...
	if (read.fail() || filesize < sizeof(LUDO)) {
		...
		} while (choice <= 0 || choice > numberofGames);
		read.seekg(0, ios::beg);
